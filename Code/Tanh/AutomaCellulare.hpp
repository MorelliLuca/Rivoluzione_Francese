...
enum class State{ POOR, RICH, REVOL, REACT, ERROR}; //Different types of people
...
class Person{
   ...
   public:
   Person(State _itsState, int _posX, int _posY): itsState{_itsState}, posX{_posX}, posY{_posY} {}; //Constructor
   //Member functions
   ...
   void changeState(State newState){ //Change state checking that the rules are followed
       ...
       if(itsState==State::POOR && newState==State::REVOL){
         ...
         itsState=newState;
       }else std::cerr<<"ERROR CHANGING STATE:"<<stateName(oldState)<<"->"<<stateName(newState)<<std::endl;
   }
};

class Nation{  //Class of the Nation to be simulatetd
   //Stuff
   Person pError{State::ERROR,0,0};  //Erorr type persone used to indicate an error
   //Member variables
...
//Member functions
   ...
   public:
   Nation( double _NR,double _NP, double _R, double _AR, double _G, double _g, double _Ag, double _beta, double _alfa) //Constructor
   : NR{_NR}, NP{_NP}, R{_R}, AR{_AR}, G{_G}, g{_g}, Ag{_Ag}, beta{_beta}, alfa{_alfa}, Population(NR+NP+R+AR,pError) {
      totPop = NR+NP+R+AR;
      sideLength = static_cast<int>(sqrt(totPop)); //Evaluating the side of the grid (USE ONLY SQUARE GRID)
      for(int i{0};i!=NR;++i){  //Creating RICH People
         int x,y;
         do{  
               x = static_cast<int>(disR(gen)*sideLength);//Generating x random coord
               y = static_cast<int>(disR(gen)*sideLength);//Generating y random coord
   ...
   Person& getPerson(double x, double y){ //Get person reference from his position in the grid
      for(int i{0};i!=totPop;++i){
         if(Population[i].getX()==x && Population[i].getY()==y) {return Population[i];}
      }
      return pError;
   }
   ...
   int countNear(double x, double y, State stateToFind){ //Count how many people of a state are present in a radius R from x,y
      int count{0};
      double center{x*sideLength+y};
       for(int i{-1};i!=2;++i){
         for(int j{-1};j!=2;++j){
            if(x+i>=0 && x+i<sideLength && y+j>=0 && y+j<sideLength && center+i+sideLength*j<totPop && center+i+sideLength*j>=0){
         if(Population[center+i+sideLength*j].getState()==stateToFind && (x!=Population[center+i+sideLength*j].getX()||y!=Population[center+i+sideLength*j].getY()) ){
            count++;
         }
            }
      }
      ...
      for(auto& it : newPopulation){
         ...
         case State::POOR:
            if(disR(gen)<g){
               ...
               }else for(int i{0};i!=this->countNear(x,y,State::REVOL);++i){
            if(disR(gen)<G){
               it.changeState(State::REVOL);
               break;
               }
